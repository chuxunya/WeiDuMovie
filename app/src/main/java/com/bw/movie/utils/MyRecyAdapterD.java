...
public class MyRecyAdapterD extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public MyRecyAdapterD(List<ReCodeBean.ResultBean> list, Context context) {
        this.list = list;
        ...
        if (holder instanceof MyHolder){
            ...
            ((MyHolder)holder).rela.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    onClickListener.onclick(list.get(position).getOrderId(),list.get(position).getPrice(),1);
                }
            });
            ...
            ((MyHolder)holder).pay.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    onClickListener.onclick(list.get(position).getOrderId(),list.get(position).getPrice(),2);
                }
            });
        }
    }
    ...
    public interface OnClickListener{
        void onclick(String orderId,double price,int id);
    }
}
