...
import com.bw.movie.utils.MyYuYueAdapter;
...
/**
 *@describe(描述)：YuYueActivity
 *@data（日期）: 2019/12/7
 *@time（时间）: 13:51
 *@author（作者）: xin
 **/
public class YuYueActivity extends BaseActivity<HomePresenter3> implements IContract3.IView {

    private static final String TAG = "YuYueActivity";
    @BindView(R.id.img_back_yuyue)
    ImageView imgBackYuyue;
    @BindView(R.id.recycler_yuyue)
    XRecyclerView recyclerYuyue;
    ...
    protected void initData() {
        ...
        Log.d(TAG, "initData: "+userId+" "+sessionId);
        mPresenter.getMyYuData(userId,sessionId);
    }
    ...
    public void onMyYuSuccess(MyYuBean myYuBean) {
        Log.d(TAG, "onMyYuSuccess: "+myYuBean);
        if (myYuBean.getStatus().equals("9999")){
            Toast.makeText(this,"请先登录",Toast.LENGTH_SHORT).show();
        }else {
            if (myYuBean.getResult()!=null){
                List<MyYuBean.ResultBean> result = myYuBean.getResult();
                LinearLayoutManager linearLayoutManager = new LinearLayoutManager(this);
                recyclerYuyue.setLayoutManager(linearLayoutManager);
                MyYuYueAdapter myYuYueAdapter = new MyYuYueAdapter(this,result);
                recyclerYuyue.setAdapter(myYuYueAdapter);
                myYuYueAdapter.setOnItemClickListener(new MyYuYueAdapter.OnItemClickListener() {
                    @Override
                    public void onItemClick(int id) {
                        Intent intent = new Intent(YuYueActivity.this, XiangActivity.class);
                        intent.putExtra("movieId",id);
                        startActivity(intent);
                    }
                });
            }else {
                Toast.makeText(this,"暂无预约电影",Toast.LENGTH_SHORT).show();
            }
        }
    }
    ...
    public void onMyYuFailure(Throwable e) {
        Log.d(TAG, "onMyYuFailure: "+e.getMessage().toString());
    }
