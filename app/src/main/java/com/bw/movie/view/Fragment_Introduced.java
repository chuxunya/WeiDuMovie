...
import com.bw.movie.utils.MyRecyAdapter_actor;
import com.bw.movie.utils.MyRecyAdapter_director;
...
 *@Description:介绍
 **/
public class Fragment_Introduced extends BaseFragment<HomePresenter> implements IContract.IView {
    private static final String TAG = "介绍";
    @BindView(R.id.summary)
    TextView summary;
    @BindView(R.id.directors)
    TextView directors;
    @BindView(R.id.recy_director)
    RecyclerView recyDirector;
    @BindView(R.id.actors)
    TextView actors;
    @BindView(R.id.recy_actors)
    RecyclerView recyActors;
    private XiangBean.ResultBean result;
    private List<XiangBean.ResultBean.MovieDirectorBean> director;
    ...
    protected int provideLayoutId() {
        return R.layout.frag_introduced;
    }
    ...
    public void onXiangSuccess(XiangBean xiangBean) {
        ...
        if (xiangBean.getResult()==null){
            ...
            director = result.getMovieDirector();
            int size = director.size();
            directors.setText("导演（" + size + "）");
            LinearLayoutManager linearLayoutManager = new LinearLayoutManager(getActivity());
            linearLayoutManager.setOrientation(RecyclerView.HORIZONTAL);
            recyDirector.setLayoutManager(linearLayoutManager);
            MyRecyAdapter_director myRecyAdapter_director = new MyRecyAdapter_director(director, getContext());
            recyDirector.setAdapter(myRecyAdapter_director);
            movieActor = result.getMovieActor();
            actors.setText("演员（" + movieActor.size() + "）");
            LinearLayoutManager linearLayoutManager1 = new LinearLayoutManager(getActivity());
            linearLayoutManager1.setOrientation(RecyclerView.HORIZONTAL);
            recyActors.setLayoutManager(linearLayoutManager1);
            MyRecyAdapter_actor myRecyAdapter_actor = new MyRecyAdapter_actor(movieActor, getContext());
            recyActors.setAdapter(myRecyAdapter_actor);
        }
    }
